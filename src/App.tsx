...
const sections = [{ id: 'about', label: 'About' }, { id: 'work', label: 'Work' }, { id: 'skills', label: 'Skills' }, { id: 'experience', label: 'Experience' }, { id: 'contact', label: 'Contact' }]
...
function App() {
  ...
  const optionalLinks = [{ label: 'GitHub', href: siteConfig.github }, { label: 'LinkedIn', href: siteConfig.linkedin }, { label: 'Resume', href: siteConfig.resume }]
  ...
  return <div className="site-shell">
    ...
    <main id="top">
      ...
      <section id="experience" className="content-section experience-section" aria-labelledby="experience-title"><div className="section-heading"><div><p className="eyebrow">03 / Experience & education</p><h2 id="experience-title">Ten years of<br /><em>building and learning.</em></h2></div><p className="section-note">From production issue resolution to architecture, mentoring and AI-ready services.</p></div><div className="experience-list">{siteConfig.experience.map((item) => <article key={`${item.company}-${item.role}`}><div><span className="eyebrow">{item.dates}</span><h3>{item.role}</h3><p className="company">{item.company}</p></div><p>{item.detail}</p></article>)}</div><div className="education"><p className="eyebrow">Education</p>{siteConfig.education.map((item) => <div key={item.qualification}><strong>{item.qualification}</strong><span>{item.school} · {item.year}</span></div>)}</div></section>
      <section className="approach-section" aria-labelledby="principles-title"><div className="section-heading"><div><p className="eyebrow">04 / Principles</p><h2 id="principles-title">Clarity is a<br /><em>technical feature.</em></h2></div><p className="section-note">I make contracts, boundaries and operational behavior explicit before optimizing the next layer.</p></div><div className="approach-flow"><div><span>01</span><h3>Understand</h3><p>Start with business context, source facts and the decision that matters.</p></div><div className="flow-arrow">→</div><div><span>02</span><h3>Design</h3><p>Make APIs, data, security and integration boundaries clear.</p></div><div className="flow-arrow">→</div><div><span>03</span><h3>Deliver</h3><p>Ship a bounded slice, observe it in production and keep improving.</p></div></div></section>
      <section id="contact" className="contact-section" aria-labelledby="contact-title"><p className="eyebrow">05 / Contact</p><h2 id="contact-title">Let’s build something<br /><em>useful together.</em></h2><p>Available for conversations about full stack development, enterprise APIs, cloud platforms and practical AI.</p><a className="button primary" href={`mailto:${siteConfig.email}`}>Email Sailesh <span>↗</span></a><div className="contact-meta"><span>{siteConfig.location}</span><a href={`tel:${siteConfig.phone}`}>☎ {siteConfig.phone}</a><a href={`mailto:${siteConfig.email}`}>{siteConfig.email}</a>{optionalLinks.map((link) => link.href ? <a key={link.label} href={link.href}>{link.label} ↗</a> : <span key={link.label} className="add-link">{link.label}: Add link</span>)}</div></section>
    </main>
